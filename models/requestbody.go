...
func GetRequestBody(requestID, requestType, subID string, types string) (map[string]string, string) {
	...
	case "identify":
		...
		requestBody["url_callback"] = "http://pl.leadernethksp.com/notification"
		requestBody["url_return"] = "http://pl.leadernethksp.com/identify/return?track=" + requestID
		if types == "unsub" {
			requestBody["url_return"] = "http://pl.leadernethksp.com/unsub/identify?track=" + requestID + "_identify" + "_" + timeStr
		}
	...
	case "start-subscription":
		...
		requestBody["url_callback"] = "http://pl.leadernethksp.com/notification"
		requestBody["url_return"] = "http://pl.leadernethksp.com/start-sub/return?track=" + requestID
		requestBody["prompt_product_args"] = `{"pic":{"img":"http://c4fun.argameloft.com/static/img/bg.png","alt":"HK Leader"},"desc":{"pl":"Click4Fun GAMES"}}`
		requestBody["prompt_merchant_args"] = `{"logo":{"img":"http://pl.leadernethksp.com/static/img/logo.png","alt":"HK Leader"}}`
		requestBody["manage_subscription_url_callback"] = "http://pl.leadernethksp.com/notification"
		requestBody["close_notification_url_callback"] = "http://pl.leadernethksp.com/notification"
	...
	case "close-subscription":
		...
		requestBody["url_callback"] = "http://pl.leadernethksp.com/notification"
		requestBody["url_return"] = "http://pl.leadernethksp.com/unsub/return?subID=" + subID
		encodeMessage = requestBody["action"] + requestBody["merchant"] + requestBody["order"] +
